...
- **Module:** `ssi_hr_expense_account_documenso_signing` is installed.
...
- Approval completes automatically — without any button click on this Employee Expense
  Account record — once the linked signature request reaches the **Signed** status
  ...
  **In Progress** automatically, and a document number is generated according to the
  sequence configuration. If there are still pending approval levels, status remains
