...
class TestUiHrReimbursement(HttpCase):
    """Tour tests for the ``hr.reimbursement`` operating unit delta."""
    ...
    def setUpClass(cls):
        ...
        does not depend on that implication chain. The multi operating
        unit group is required for the Operating Unit field itself to
        be rendered on the form.
        """
        super().setUpClass()
    ...
    def test_field_ou(self):
        """Run the delta tour for the ``hr.reimbursement`` create form.
