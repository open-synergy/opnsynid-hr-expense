...
>
> **Extends:** ssi_hr_cash_advance — model `hr.cash_advance`, aksi `05-approve`
...
- **Config:** The record's active Approval Template has a Documenso Signing Template
  ...
  **Approve** / click **OK**) applies unchanged, exactly as documented in
  `ssi_hr_cash_advance/docs/hr_cash_advance/05-approve.md`.
...
- Approval completes automatically — without any button click on this Employee Cash
  Advance record — once the linked signature request reaches the **Signed** status
  (checked manually via **Check Status** on the request, or synced automatically by the
  connector). At that point, the base Post-Condition applies as usual: status changes to
  **Open**, and a journal entry is automatically created in the configured journal. If
  there are still pending approval levels, status remains **Waiting for Approval**.
