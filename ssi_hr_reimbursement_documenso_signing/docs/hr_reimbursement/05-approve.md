...
- **Config:** The record's active Approval Template has a Documenso Signing Template
  ...
  **Approve** / click **OK**) applies unchanged, exactly as documented in
  `ssi_hr_reimbursement/docs/hr_reimbursement/05-approve.md`.
- **Module:** `ssi_hr_reimbursement_documenso_signing` is installed.
...
- Approval completes automatically — without any button click on this Employee
  Reimbursement record — once the linked signature request reaches the **Signed** status
  ...
  **In Progress** automatically, and an accounting journal entry is created
  automatically. If there are still pending approval levels, status remains **Waiting
  for Approval**.
