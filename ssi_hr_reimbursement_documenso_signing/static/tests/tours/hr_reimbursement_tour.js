...
odoo.define("ssi_hr_reimbursement_documenso_signing.hr_reimbursement_tour", function (
    require
) {
    "use strict";
    ...
    // ssi_hr_reimbursement/docs/hr_reimbursement/05-approve.md Flow steps
    // 1-2 -- see skill odoo-development-ui-test, scope-and-boundaries.md
    // §3 ("E2a -- telusur-ulang aksi itu dari base sampai titik ubah").
    // The delta assertion, anchored at base Flow step 2 (open the
    // record to approve), verifies the Signature Requests tab injected
    // because `_documenso_signing_create_page = True`, then stops --
    ...
    // driven by the external Documenso connector and is out of scope
    // for a tour -- it never calls the external Documenso service and
    // never triggers creation of a signature request (Keputusan
    // Desain, issue open-synergy/opnsynid-hr-expense#113).
    tour.register(
        ...
        [
            ...
            // ── Base Flow 2 — Open the record to approve.
            {
                ...
                trigger:
                    ".o_data_row:contains(Tour Reimbursement Documenso Employee Approve) .o_data_cell:first",
                extra_trigger: ".o_list_view",
            },
            {
